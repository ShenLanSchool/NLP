...
class FastLineBinaryReader(file: String, skipBytes: Long = 0, encoding: String = "ISO-8859-15") extends Iterator[String] {
  //private var in = new FileReader(file)
  private var sb: StringBuilder = null
  private var line: String = null
  private var in = new DataInputStream(new FileInputStream(file))
  //println("Default charset of this JVM : " + in.getEncoding() + ", so using " + in.getEncoding() +  " to read the input file")
  //in.skip(skipBytes)
  moveToNext()
  ...
  private def moveToNext(): Unit = {
    sb = new StringBuilder()
    var c = in.readChar() // read char . Internally, read 1 byte or 2 byte or 3 bytes depending encoding.
    // go inside only if c is bad char
    while (c != -1 && c == '\n') c = in.readChar()
